...
async def _persist_elicitation_request_item(
    ...
    ``elicitation_id``. Never raises — a persistence failure must not
    block the live publish that already happened.
    """
...
async def _persist_elicitation_resolved_item(
    ...
    Never raises — a persistence failure must not block the live
...
async def _apply_profile_or_rollback_session(
    ...
    try:
        ...
        except Exception:  # noqa: BLE001 — rollback itself must never hide the loader error
            _logger.exception(
